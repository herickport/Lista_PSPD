...
#define OUTFILE "out_julia_mpi_io.bmp"
...
int main(int argc, char *argv[]) {
    ...
    MPI_Offset offset = start * largura * 3;

    // Escrita não bloqueante
    MPI_Request request;
    MPI_File_iwrite_at(file, offset, pixel_array, area, MPI_CHAR, &request);

    // Escrita bloqueante
    // MPI_File_write_at(file, offset, pixel_array, area, MPI_CHAR, MPI_STATUS_IGNORE);
